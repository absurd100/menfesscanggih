...
# ==========================================
TOKEN = os.getenv("BOT_TOKEN", 'isitokenmu')
DEFAULT_CHANNEL = os.getenv("CH_ID", 'isi id channelmu') 
MAIN_OWNER_ID = isi id ownermu
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("ban_") or data.startswith("unban_"):
        ...
        try:
            # Pengecekan aman apakah log adalah Foto atau Teks murni
        ...
        await query.message.reply_text("📝 <b>Kirim template postingan baru.</b>\nPastikan mengandung <code>{TEXT}</code> dan <code>{SENDER}</code>.", parse_mode=ParseMode.HTML)
    elif data == "cp_ch":
        if user_id != OWNER_ID: return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- PEMFORMATAN & PENGIRIMAN MENFESS FINAL ---
    ...
    if raw_text_input == '💳 Isi Kuota':
        ...
        # Jika owner sudah menyetel link gambarnya, bot akan mengirimkan gambarnya dulu
        if img_qris and img_qris != "Belum disetel":
            try:
                ...
                # Fallback jika link gambar rusak/error, tetap kirimkan instruksi teks murninya
